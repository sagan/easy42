...
type Node struct {
	...
	Metric        *int              `json:"metric,omitempty"`         // Global metric when bird export routes to kernel
...
// ExternalIp6 returns ExternalIP6 if set, or IP6 if not set
func (n *Node) ExternalIp6() string {
	if n == nil {
		return ""
	}
	if strings.TrimSpace(n.ExternalIP6) != "" {
