...
export const AddNodeModal: React.FC<AddNodeModalProps> = ({
  ...
  const handleUpdateEntrypoint = (id: string, field: keyof EditableEntrypoint, value: string) => {
    setEntrypoints((prev) => prev.map((e) => (e.id === id ? { ...e, [field]: value } : e)));
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (isExternal) {
      if (!name.trim() || !asn) {
        setSaveError("Node Name and ASN are required for external peer");
        return;
      }
    } else {
      ...
      <DialogTitle sx={{ display: "flex", alignItems: "center", gap: 1.5, pb: 1, borderBottom: "1px solid #E2E8F0" }}>
        <Box
          sx={{
            ...
            height: 34,
            borderRadius: 2,
            backgroundColor: isExternal ? "rgba(139, 92, 246, 0.1)" : "rgba(79, 70, 229, 0.1)",
            display: "flex",
            alignItems: "center",
            justifyContent: "center",
            color: isExternal ? "#8B5CF6" : "#4F46E5",
          }}
        >
          {isExternal ? <Globe size={18} /> : nodeToEdit ? <Edit2 size={18} /> : <Server size={18} />}
        </Box>
        <Typography variant="h6" sx={{ fontWeight: 700, color: "#0F172A" }}>
          {nodeToEdit
            ? isExternal
              ? `Edit External Peer: ${nodeToEdit.name}`
              : `Edit Node: ${nodeToEdit.name}`
            : isExternal
              ? "Add External Peer (DN42)"
              : "Add New Mesh Node"}
        </Typography>
      </DialogTitle>
          ...
          {isExternal ? (
            <Box sx={{ display: "flex", flexDirection: "column", gap: 2 }}>
              <Alert severity="info" sx={{ borderRadius: 2 }}>
                External peers represent unmanaged private or DN42 network nodes. easy42 does not connect via SSH to
                this node; it is only used to configure WireGuard and BGP peering on your managed nodes.
              </Alert>
              ...
              <Box sx={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 2 }}>
                ...
                <TextField
                  label="Peer AS Number (ASN)"
                  type="number"
                  size="small"
                  value={asn}
                  onChange={(e) => setAsn(Number(e.target.value))}
                  helperText="Remote peer's DN42 or public ASN"
                  required
                  disabled={saving}
                />
              </Box>
              ...
              <TextField
                fullWidth
                size="small"
                label="Description / Peering Partner"
                placeholder="e.g. DN42 peer with AS4242421234 in Frankfurt"
                value={description}
              ...
              <Box sx={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 2 }}>
                ...
                <TextField
                  ...
                  placeholder="e.g. dn42, external, frankfurt"
            ...
            <>
              ...
              <Box>
                <Typography
                  variant="caption"
                  sx={{ color: "#475569", fontWeight: 700, mb: 0.8, display: "block", letterSpacing: "0.5px" }}
                >
              ...
              <Box sx={{ display: "flex", flexDirection: "column", gap: 2 }}>
                <Typography
                  variant="caption"
                  sx={{ color: "#475569", fontWeight: 700, display: "block", letterSpacing: "0.5px" }}
                >
                ...
                <Box sx={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 2 }}>
                  ...
                  <TextField
                    ...
                    value={asn}
                    onChange={(e) => setAsn(Number(e.target.value))}
                    helperText="e.g. 4224420001 or DN42 ASN"
                    required
                    disabled={saving}
                  />
                </Box>
                        ...
                        >
                          <Box sx={{ display: "flex", alignItems: "flex-start", gap: 1, flex: 1, minWidth: 0 }}>
                            ...
                            <Box>
                              <Typography
                                variant="body2"
                                sx={{ color: "#B45309", fontWeight: 700, fontSize: "0.8rem" }}
                              >
                      ...
                      >
                        <Box sx={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                          <Box sx={{ display: "flex", alignItems: "center", gap: 0.8 }}>
                            ...
                            <Typography
                              variant="subtitle2"
                              sx={{ fontWeight: 700, fontSize: "0.8rem", color: "#0F172A" }}
                            >
                              Direct Entrypoint
                            </Typography>
                          </Box>
